...
- For an implementation-and-delivery request, successful completion is the requested change verified as reasonably possible, pushed, and represented by an opened or updated pull request. Once that is true, stop and report delivery. Do not watch CI, review bots, pull-request checks, or reviews; do not run poll/sleep loops after pushing; and do not wait for post-delivery feedback.
- Do not automatically create or request kernel CI/review continuations, completion heads, checker workers, or external-condition gates after delivery. The user will explicitly retrigger or request follow-up work. Preserve a specifically requested post-delivery action such as CI remediation or responding to review, but do not infer additional monitoring beyond it.
...
An ordinary implementation-and-delivery request ends when the worker has verified the change as reasonably possible, pushed it, and opened or updated the pull request. Heads must not automatically attach a completion head, checker worker, CI/review continuation, or external-condition gate, and must not ask the delivery worker to watch CI, review bots, checks, or reviews. The user will explicitly retrigger follow-up work. A head may route CI remediation, review response, merge/deploy monitoring, or another post-delivery action only when the user specifically requests that action.
...
Users should be able to navigate to the agent tree using a keybind and JUMP into the worker's underlying coding harness session in a new terminal, tmux pane, or other configured launcher. Harness-specific resume and attach behavior stays behind the harness integration layer.
...
`SpawnWorker.workspace_mode` is explicit and persisted as `isolated` (default) or `shared_read_only`. For `shared_read_only`, the kernel uses only a clean, readable, registered, unlocked, non-bare checkout on a branch, and only when the selected harness advertises enforcement. A clean `main`/`master` checkout is preferred; failing that, a clean checkout whose commit is already on mainline; failing that, a clean feature branch. The choice is persisted on the workspace plan as `shared_checkout_selection` (`main_branch`, `mainline_snapshot`, `other_branch`), and the spawn log line and workspace note name the branch whenever it is not `main`/`master`, so the user knows which code a reader's findings describe. Detached checkouts are never shared. Registered bare roots may select an existing linked checkout but never the bare root itself. When a bare root has no suitable linked checkout, the workspace manager creates a deterministic manager-owned main/master checkout under its workspace root, with durable ownership and cross-process provisioning locks, and retains it as a reusable cache. Concurrent shared readers are allowed. The kernel revalidates immediately before launch; unsafe, dirty, moved, or failed caches are refused, and only provably incomplete owned cache creation may be recovered destructively. When no suitable checkout can be created or validated, it safely falls back to ordinary isolated allocation and persists `effective_workspace_mode: isolated` plus a fallback reason. Shared checkouts are never removed during worker cleanup.
For git-backed projects, the preferred allocation strategy is one git worktree per worker using a Meringue-owned, human-facing branch name derived from the issue/task title, such as `fix-signup-validation`. If occupied, deterministic numeric candidates such as `fix-signup-validation-2` and `-3` preserve isolation. Do not expose Meringue agent ids, worker ids, harness session ids, or subagent implementation details in workspace branch names. Candidate path/branch ownership must be durably reserved under a per-candidate cross-process lock before Git or filesystem mutation. Before harness launch the kernel must revalidate directory editability, non-bare status, Git registration/branch/lock state, durable owner identity, and live-worker occupancy; an unusable or colliding managed candidate is excluded and safely reallocated rather than shared.
...
A prompt RPC timeout is not proof that the prompt failed: a harness can persist the request after
its acknowledgement deadline (Pi does this while compacting large resumed transcripts). Receipt-
capable clients must give the prompt a durable delivery id. Reconciliation must not poll, resume, or
deliver a later prompt while that id is absent and the original process is live; it confirms from
the saved transcript without replay, or retries only after the process is gone and durable absence
proves the original request can no longer land.
