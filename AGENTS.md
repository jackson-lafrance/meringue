...
Before every worker push, fetch the target base branch and compare its commit with the last delivery baseline. If the base advanced, rebase all worker commits onto the latest base before pushing, even when the pull request reports no conflicts. If the base is unchanged, skip the rebase. If the rebase conflicts, preserve the workspace and report the exact conflict; never bypass the rebase or force-push the worker branch.
