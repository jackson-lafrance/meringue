...
There are three ways to focus a worker, and which one applies is the backend's answer, not a harness name the UI keeps a list of. `Harness::Client#live_terminal_supported?`, `#managed_session_view_supported?`, and `#interactive_session_supported?` decide; `Workspace::Controller#focus_mode` asks the kernel, which asks the client.
...
4. Dashboard-issued prompts are queued while the marker is live, because they would be typed into the same box the user is typing into. Reconciliation delivers them after focus releases the marker.
...
### Native attach (managed-transport backends)

Pi's native TUI can open the same durable session that Meringue manages through RPC. Focusing a Pi worker is therefore a **native session attach**, not a process replacement or a prompt handoff:

1. The kernel durably claims the worker's `interactive_handoff` before process I/O. The claim includes the worker/issue ids, session identity, workspace path/branch, and owner process identity. A repeated request is rejected while this claim is live, so only one focused PTY can open.
2. `PiClient` reads authoritative RPC state, preserves the existing session id and JSONL file, resolves the configured executable through the effective `PATH`, and builds a native argv without `--mode rpc` or trailing text. Preparation sends no prompt, abort, steer, follow-up, or continuation.
3. The workspace controller launches the real `pi --session …` InteractiveMode process in a PTY that occupies the dashboard logs pane. The existing managed process remains live, so an active turn continues instead of being aborted. The native process is never given synthetic input on focus entry.
4. While the PTY owns the focused pane, reconciliation, dashboard prompts, and pause/resume side effects stand down for that worker. Dashboard prompts and lifecycle requests remain durable and deliver after native focus ownership returns; they are not rejected because the user is inside Agent session. AgentTree, external dashboard chat, and status updates remain live. The PTY receives each logs-pane resize.
5. Focused keyboard and mouse-wheel input reaches the native Pi application when its pane has focus. Page keys and retained focused-transcript rows use the workspace scroll offset when Meringue owns the rendered rows. The workspace leader remains dashboard-owned, so controls and pane switching do not leak bytes into Pi.
6. When native focus ownership returns, the controller closes its focused native PTY and the kernel reads the same managed session again. It sends no continuation or other automatic message. The durable marker clears after dashboard ownership returns, then queued prompts and lifecycle operations reconcile through the normal single transport.
7. If Meringue crashes during focus, startup claims the stale marker, checks the recorded native pid against the configured command, reclaims that native process when safe, and resumes the same saved session without inventing a continuation. A reused or unrelated pid is never signaled; a failed recovery keeps the marker retryable.

The former external session opener remains available only when a native focused session is not active. It is never used as a second focused writer. A backend that offers neither a live terminal nor native attach keeps the read-only session view.
...
1. Register a large repository at a normal main checkout, spawn a `shared_read_only` informational worker, and confirm it starts immediately in that checkout without a new directory/branch. Start a second reader and confirm both use the same path. Confirm Pi exposes only read/grep/find/ls and refuses write/edit/bash calls. Switch that checkout to a clean feature branch and spawn another reader: confirm it still starts there, and that the spawn log line and the worker's `workspace_note` name the branch. Repeat with a bare registered root: confirm an existing linked main checkout is selected over a linked feature-branch checkout, then remove the main checkout and confirm the feature-branch checkout is selected and named; remove that too and confirm Meringue falls back to an isolated non-bare worktree rather than launching in the bare repository.
...
9. Exercise the complete Agent session flow. First launch Meringue with `pi` available only in its inherited `PATH`; then repeat with a restricted inherited `PATH` and Pi available only through `[harness.pi.env] PATH`; finally use an absolute `[harness.pi] command`. Open Agent session during an active managed turn and confirm the turn continues, no abort or prompt RPC occurs, and the native command contains the existing session without `--mode rpc` or trailing text. Confirm Pi occupies only the logs pane; type a prompt and a Pi slash command there, resize the terminal, and verify interaction and wrapping remain correct. Use the top-border controls, dashboard chat, AgentTree, and mouse wheel; confirm those panes stay live and their keystrokes never reach Pi. Send a dashboard prompt while the native pane is focused and confirm it queues without an `interactive_focus_active` rejection, then close focus and confirm the same prompt delivers once through the managed session. Leave with leader + `q`; confirm the dashboard resumes without an automatic continuation or duplicate prompt. Finally, repeat with a completed worker whose RPC process is already gone, with cancellation during pending focus, with an unavailable command, and after relaunching Meringue during focus; confirm saved-session attach, ordinary failure recovery, and stale-marker recovery preserve the same session identity.
...
11. Scroll a long Meringue-rendered transcript with the wheel and `PageUp`/`PageDown`; confirm its bounded outer offset remains smooth. Then focus representative Pi and Claude workers with enough native history to overflow the embedded pane. Confirm the wheel over the harness pane and `PageUp`/`PageDown` while that pane has focus move the harness application's own history (not a captured Meringue viewport), dashboard chat/tree input still does not leak into the harness, and typing in dashboard chat remains responsive while the session stays open. The bounded regression checks live in `test/integration/workspace/terminal_scroll_performance_test.rb` and `test/integration/tui/workspace_lifecycle_test.rb`.
...
10. Outside Agent session, press leader + `a` and confirm the external launcher receives the selected worker record. During Agent session it should show the already-open notice rather than create a second writer.
...
12. Scroll a long Meringue-rendered transcript with the wheel and `PageUp`/`PageDown`; confirm its bounded outer offset remains smooth. For an embedded harness, confirm the same events navigate the harness application's native history and never create an outer focused-session offset.
