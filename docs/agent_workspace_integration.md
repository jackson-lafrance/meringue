...
3. The workspace controller launches the real `pi --session …` InteractiveMode process in the PTY-backed focus screen. It prefers the same persisted JSONL session and an explicit continuation prompt when the RPC turn was interrupted. If the original file is unavailable but the live RPC process can still export entries, Pi writes a version-3 replacement JSONL session from the complete entry stream, records the replacement/source ids, and both native focus and dashboard RPC use that replacement thereafter.
