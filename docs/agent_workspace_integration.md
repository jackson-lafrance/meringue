...
6. On focus close, Meringue restores the dashboard panes immediately while a controller-owned thread closes the Agent session and then reattaches the dashboard RPC client to the same session file. It sends Pi's Escape interrupt first because that cancels active turns and autocompaction; Ctrl-C only clears Pi's editor. The durable handoff marker keeps reconciliation and managed prompting out until reattachment finishes, and application shutdown joins any outstanding return thread rather than abandoning ownership. If Agent session produced a newer final assistant result, dashboard ownership returns settled. If it exited with a pending tool call or no newer final result, the dashboard automatically sends the saved continuation and returns the worker to `working` before reconciliation can classify the intentionally interrupted turn. No manual recovery prompt is required.
...
If Meringue crashes during handoff, after Agent session starts, or while focus is returning, the persisted marker keeps settlement under the handoff lifecycle. Startup reconciliation claims a stale marker before polling, verifies and reclaims any surviving interactive pid through the configured-command identity check, then resumes the same dashboard session and its saved continuation. A reused or unrelated pid is never signaled. If reclaim or resume fails, the marker remains retryable and ordinary settlement polling stays suppressed, so an aborted pre-focus assistant checkpoint cannot manufacture a completion. This closes the PTY-launch race, crash/relaunch path, and ordinary focus lifecycle without allowing a second writer.
...
9. Exercise the complete Agent session flow. First launch Meringue with `pi` available only in its inherited `PATH`; then repeat with an intentionally restricted inherited `PATH` and Pi available only through `[harness.pi.env] PATH`; finally use an absolute `[harness.pi] command`. In each case open Agent session while a managed prompt/tool call is active and confirm Pi receives one abort, the managed turn settles before its RPC writer exits, and Agent session becomes the only session owner. Confirm Pi occupies only the logs pane; type a prompt and a Pi slash command there, resize the terminal, and verify interaction and wrapping remain correct. Use the top-border terminal, editor, and dashboard controls and the mouse wheel, then click/tab through AgentTree and dashboard chat, change selection, route an external chat message, and monitor other worker/log/status updates; confirm those panes stay live and their keystrokes never reach Pi. Return focus to logs and confirm Pi still accepts input. Leave with leader + `q` before the continuation finishes; confirm the prior logs mode returns immediately, keyboard focus is usable, the dashboard reattaches once in the background, automatically continues the saved assignment, keeps the worker `working`, and reconciliation does not mark the pending tool turn errored. Trigger `/compact`, leave while the compaction indicator is active, and confirm Escape cancels compaction without freezing or exiting Meringue. Repeat after allowing Agent session to produce a final result and confirm the dashboard does not send a duplicate continuation. Finally, repeat with a completed worker whose RPC process is already gone, with cancellation during pending handoff, with an unavailable command, and by terminating Meringue after focus opens before relaunching it; confirm saved-session focus opens in the first case, ordinary failures restore the previous dashboard without a second owner or frozen input, and crash recovery resumes the unfinished continuation rather than completing it from the aborted checkpoint.
