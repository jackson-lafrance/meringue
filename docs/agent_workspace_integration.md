...
1. The kernel durably claims the worker's `interactive_handoff` before process I/O. The claim includes the worker/issue ids, issue title and description, original assignment, workspace path/branch, session identity, and owner process identity. A repeated request—whether from the same dashboard or a second instance—is rejected while this claim is live, so interruption and ownership transfer happen exactly once.
2. `PiClient` snapshots already-available retained events and session-file context. It does not wait for an active prompt or tool call to finish and does not add an abort/settle RPC round trip; interactive focus directly terminates the managed RPC process on a short bound. If a resumable settled worker already has no RPC process, it is treated as already quiesced rather than attempting to terminate a missing writer.
3. Meringue records a bounded handoff summary, resolves the native environment, releases transport ownership, and terminates the RPC process when one exists. No native process starts while a managed writer is alive.
4. The workspace controller launches the real `pi --session …` InteractiveMode process in the PTY-backed focus screen. It prefers the same persisted JSONL session and supplies an explicit continuation prompt when the RPC turn was interrupted. That prompt tells Pi to inspect the transcript and workspace before repeating tools and carries bounded assignment, issue, latest-intent, assistant-progress, path, and branch context. If the original file is unavailable but the live RPC process can still export entries, Pi writes a version-3 replacement JSONL session from the complete entry stream, records the replacement/source ids, and both native focus and dashboard RPC use that replacement thereafter.
5. While the PTY owns the session, reconciliation and managed prompting stand down. Meringue's workspace leader/navigation keys remain outside Pi and are consumed before ordinary bytes are forwarded to Pi.
6. On focus close, Meringue closes the native process first, then reattaches the dashboard RPC client to the same session file. The marker is cleared only after reattachment succeeds; a failed resume remains retryable instead of being reported as a clean completion. A launch that never starts also reattaches RPC, and settled returns become `idle` rather than leaving a misleading `working` worker. The bounded claim/handoff is retained as `last_interactive_handoff` for diagnostics and continuity after active ownership is released.

If Meringue crashes after native Pi starts, the persisted marker records the interactive pid and start time. A later owner verifies that pid still matches the configured Pi executable, reclaims it, and only then resumes the same session; a reused or unrelated pid is never signaled. This closes the small PTY-launch race as well as the ordinary focus lifecycle without allowing a second writer.

This preserves the single-writer invariant and keeps worker state, session history, PTY output, latest user intent, and dashboard return behavior testable. It does **not** preserve an in-flight provider request or exact token/tool-stream cursor: Pi has no public checkpoint for that state, and terminating an active turn can discard non-durable partial output. The continuation prompt is intentional and may cause Pi to inspect or repeat work; the bounded handoff record is diagnostic context, not a second transcript. A future upstream session lease/checkpoint protocol could remove that limitation.
...
9. Open native Pi focus while a managed prompt/tool call is active. Confirm the switch is immediate, ordinary keys reach Pi, leader/navigation controls remain Meringue-owned, and leader + `q` quiesces Pi before the dashboard RPC session is reattached. Confirm the interrupted turn is reported as a process-termination handoff rather than falsely claimed as an exact stream transfer. Repeat with a completed worker whose RPC process is already gone and confirm native focus still opens from its saved session.
