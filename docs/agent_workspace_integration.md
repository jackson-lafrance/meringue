...
5. Run `/prune` with open, closed, merged, and forge-unavailable PRs. Confirm only bundles whose PRs are settled (merged or closed) are eligible. For an eligible worker, confirm a clean managed worktree is removed while its branch remains; then repeat with dirty and locked worktrees and confirm the bundle/worktree remain with a warning log until cleaned/unlocked and retried. Confirm a manually missing or already-removed worktree is handled idempotently.
