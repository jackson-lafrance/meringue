...
Workers receive standing guidance that they do not need to ask for user permission before editing files, committing, pushing, or opening/updating a PR when the assigned issue asks for those actions. Do not add worker prompts that tell them to wait for routine git/PR approval; do include requested delivery actions in the prompt. Workers verify the assigned checkout and stop writes on an ownership mismatch so the kernel—not the worker—can reallocate; they never create a nested replacement worktree. For environment problems they first exhaust repository setup guidance, safe dependency repair, available environment/tool discovery, bounded transient retries, and narrower verification. If full verification remains unavailable, they report exact failed commands and still deliver unless repository guidance explicitly forbids it. True blockers remain missing auth, invalid remotes, unresolved ownership collisions, unrelated work that would be overwritten, or unsafe/destructive operations.
