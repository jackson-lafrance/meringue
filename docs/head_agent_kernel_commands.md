...
- `GetSessionDefaults` reports the future-session head and worker model and thinking levels and takes `{}`. It has no
  slash command: the dashboard status line already shows `Pi defaults: <model> · head <level> · worker <level>`
  (splitting the model by role when they differ) and `/config` prints each role's model and thinking level, so the
  typed `/defaults` was removed. Propose it when the user asks about the defaults in natural language.
