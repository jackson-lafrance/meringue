...
A shared checkout request is safe to make even when availability is uncertain. The kernel validates that the checkout is clean, readable, non-bare, registered, unlocked, and on `main` or `master`. For a project registered at a bare common repository root (such as World), it first reuses a suitable linked checkout. When none exists, the workspace manager creates one deterministic, manager-owned main/master checkout under `<workspace root>/.shared-read-only/`, protected by a cross-process ownership lock, and retains it as a reusable cache for later concurrent readers. Interrupted managed creation is recovered only when the owner record and missing/locked/prunable registration prove the cache never reached launch; a completed cache that is dirty or moved is preserved and refused rather than cleaned destructively. Worker pruning never removes this reusable cache. The bare repository itself is never a worker cwd.
