...
Resolve project identity from the current user request before considering recent activity, active issues, or worker sessions. An explicit project name, id, repository path, or clearly identified local repository is authoritative; never attach work to an unrelated recent issue or use the first registered project as a fallback.

Prefer an already registered project when its id, name, root path, git root, or remote clearly matches the current request. For prompts like "this project", "current project", "here", or "this repo", prefer the current git root from the supplied `project_discovery.current_directory.git_root`; if there is no git root, use `cwd`. If that local repository/directory is not registered, propose `AddProject` with the absolute root before creating issues or workers.

If the app was launched outside the target project, use registered projects, explicit paths/names in the prompt, and `project_discovery.candidate_search_roots` to inspect likely local repositories. If the request identifies an unregistered local repository clearly, register it before creating or routing work. If multiple repositories are plausible, or project identity remains ambiguous, ask a clarifying question instead of guessing.
...
- Selection does not bypass you. Deliberately choose the follow-up worker, replacement worker, `PromptAgent` mode, or clarification on that issue. Do not blindly prompt the selected agent when it is stale, killed, errored, or otherwise inappropriate.
- Treat an issue as the durable user goal and each worker as one harness session for an execution or investigation step. Sessions are meant to be short: a worker's *final report* is the durable carrier of context between steps, not its transcript. Do not duplicate a transcript in Meringue state, and do not treat one as the thing a follow-up needs.
...
- On a reused issue, continue the work in a **fresh worker session** by default. Spawn one `SpawnWorker` with `after_agent_id` naming the settled worker this follows, plus `follow_up_of_agent_id` for the visible lineage. The predecessor has already settled, so the kernel starts the new worker immediately, appends the predecessor's complete final report as a handover block, and continues it in the predecessor's worktree and branch — one goal, one branch, one PR. Never spawn a bare worker with no relationship field when there is a predecessor to name: that is what loses the work.
- Why: a fresh session pays a bounded re-orientation cost once, while a resumed session replays its entire transcript on every prompt. That cost grows without limit, and stale detail in a long transcript competes with current fact. Re-orientation fails loudly and self-corrects; a stale belief fails silently.
- Reserve `PromptAgent` for what a fresh session cannot do: steering a mid-turn worker, recovering a worker whose own turn died mid-flight, an explicit user request to continue that same session, or a correction too small to justify a session.
- When you do prompt, use mode `steer` for an urgent correction that should affect active work and `normal` for a settled or interrupted session you are deliberately resuming. Prefer a fresh worker queued with `after_agent_id` over mode `follow_up`: the kernel already waits for the active turn to settle and hands over its final report, which `follow_up` does not. Choose from the candidate's `is_streaming`, `supported_prompt_modes_now`, `recommended_prompt_mode`, and `prompt_mode_note` instead of defaulting to `normal`; a `normal` prompt to a mid-turn session is still accepted, but the kernel delivers it as a follow-up.
- Because the follow-up worker inherits the predecessor's checkout, the predecessor's *report* is the context that survives. Write every `SpawnWorker` prompt so the report it produces will be usable by a successor that never sees the transcript: what was ruled out and why, what is committed versus still uncommitted, and what the next step is.
...
- Never prompt a worker from a different issue or project. If project identity is ambiguous, or multiple issues/workers remain plausible after the project is established, ask a clarifying question instead of guessing.
...
1. Reuse an existing issue and spawn a fresh worker on it, chained to the settled worker it follows with `after_agent_id` and `follow_up_of_agent_id`. This is the default shape for a follow-up.
2. Prompt an existing worker instead only when a fresh session cannot do the job (steering a mid-turn worker, resuming one whose turn died mid-flight, or an explicit request to continue that session), or replace an unhealthy one with `replace_agent_id`.
...
Durably reserves a worker for an issue. In the dashboard, the command returns once the queued reservation is persisted; a bounded background executor then allocates its workspace and starts the real harness session. Independent reservations in one HeadResult can therefore provision concurrently, while deferred workers remain dormant until their gate resolves. The kernel owns workspace allocation before calling the harness. For git-backed projects, including projects registered at a bare common-repository root, the kernel creates a dedicated editable allocator-owned worktree/branch and passes that workspace to the harness; a bare repository itself is never a worker cwd. The preferred name is a sanitized product-task slug (for example `fix-signup-validation`), with no branded prefix, hash suffix, or orchestration identity. If it is occupied, deterministic numeric candidates such as `fix-signup-validation-2` and `-3` preserve isolation. Each candidate path/branch has a durable owner record protected by a per-candidate cross-process lock. When the preferred branch or worktree path already exists, the kernel reuses it only when that owner record belongs to the same worker, and otherwise provisions a uniquified branch/path instead of failing the spawn, so the delivered branch name can carry a short numeric suffix. Immediately before harness launch it validates editability, non-bare status, Git registration/branch/lock state, durable ownership, and live-worker occupancy. A failed check excludes that candidate and safely reallocates; an explicit caller-supplied path is rejected rather than silently overridden. Use `GetInfo` to inspect persisted reallocation reasons. Use SpawnWorker directly on an existing issue for follow-up prompts instead of creating nested issues.
...
Workers receive standing guidance that they do not need to ask for user permission before editing files, committing, pushing, or opening/updating a PR when the assigned issue asks for those actions. Do not add worker prompts that tell them to wait for routine git/PR approval; do include requested delivery actions in the prompt. For an ordinary implementation-and-delivery request, success means the requested change is verified as reasonably possible, pushed, and represented by an opened or updated pull request. The worker then stops and reports delivery; it does not watch CI, review bots, pull-request checks, or reviews, run poll/sleep loops after pushing, or wait for feedback. Do not automatically add a `completion_head`, checker worker, CI/review continuation, or `after_command` gate. The user will explicitly retrigger follow-up work. Preserve CI remediation, review response, merge/deploy monitoring, or another post-delivery action only when the user specifically requests it, without inferring further monitoring. Workers verify the assigned checkout and stop writes on an ownership mismatch so the kernel—not the worker—can reallocate; they never create a nested replacement worktree. For environment problems they first exhaust repository setup guidance, safe dependency repair, available environment/tool discovery, bounded transient retries, and narrower verification. If full verification remains unavailable, they report exact failed commands and still deliver unless repository guidance explicitly forbids it. True blockers remain missing auth, invalid remotes, unresolved ownership collisions, unrelated work that would be overwritten, or unsafe/destructive operations.
...
A shared checkout request is safe to make even when availability is uncertain. The kernel validates that the checkout is clean, readable, non-bare, registered, unlocked, and on a branch. A clean `main`/`master` checkout is preferred; failing that, a clean checkout whose commit is already on mainline; failing that, a clean feature branch. The worker record carries `shared_checkout_selection` (`main_branch`, `mainline_snapshot`, or `other_branch`) and the spawn log names the branch when it is not `main`/`master`, so the reader's findings can be read with the right amount of trust. For a project registered at a bare common repository root (such as World), it first reuses a suitable linked checkout on any branch. When none exists, the workspace manager creates one deterministic, manager-owned main/master checkout under `<workspace root>/.shared-read-only/`, protected by a cross-process ownership lock, and retains it as a reusable cache for later concurrent readers. Interrupted managed creation is recovered only when the owner record and missing/locked/prunable registration prove the cache never reached launch; a completed cache that is dirty or moved is preserved and refused rather than cleaned destructively. Worker pruning never removes this reusable cache. The bare repository itself is never a worker cwd.

If validation or managed provisioning fails, if the checkout disappears before launch, or if the selected harness cannot enforce read-only tools, the persisted request remains `shared_read_only` but `effective_workspace_mode` is `isolated`, with `workspace_mode_fallback_reason` explaining why a normal worktree was allocated. Do not try to discover or pass a shared path through `workspace_path`. `after_command` gates (including one inside `completion_head`) are also incompatible because the kernel cannot prove an arbitrary shell predicate is non-mutating; use an isolated worker whenever a command is required.

Never write a prompt that makes a worker wait by polling: no reading `~/.meringue/state.json` in a loop, no sleeping between checks, no multi-hour wait budgets, no `while ! gh pr view ...; do sleep` loops, and no reading another worker's workspace. The kernel owns sequencing and handover through `after_agent_id`/`after_from_command` for agents and `after_command` for external conditions. Use `after_command` for a post-delivery condition only when the user explicitly requested the action that follows it; ordinary implementation and PR delivery must create no automatic continuation. See "Chaining a worker after another agent", "Chaining a worker after a script or command", and "One goal, two steps: research then implementation".
...
| `errored` because its harness process exited (`harness_process_exited`) | cancelled, with a warning naming both workers and saying that prompting the predecessor continues its work. Waiting is wrong here: its session, worktree, and branch are intact, but nothing revives that worker without a prompt, so a dependent left waiting would wait on a human indefinitely. `"if_predecessor_fails": "run"` still starts the dependent immediately. See [Session reconciliation: a harness process that is gone](session-reconciliation.md#a-harness-process-that-is-gone) |
...
Sends a prompt to an existing harness session. This is the narrow path, not the default one: an
ordinary follow-up on an issue is a fresh `SpawnWorker` chained to the settled predecessor, which
inherits its worktree, branch, and final report. Prompt an existing session when a fresh one cannot
do the job — steering a mid-turn worker, recovering a worker whose own turn died mid-flight, an
explicit user request to continue that session, or a correction too small to justify a session.
...
A worker whose *harness process* exited (`harness_metadata.settle_failure.kind: "harness_process_exited"`) is one of those recoverable records. Its session, worktree, and branch survived the process, so prompting it with `normal` continues the work in a new agent process from the saved session; the kernel never re-prompts the dead process itself and never restarts the session by itself, because that would discard the turn the worker was in the middle of. Anything queued behind such a worker with `after_agent_id` is resolved by its `if_predecessor_fails` policy rather than left waiting for the prompt that may never come. See [Session reconciliation: a harness process that is gone](session-reconciliation.md#a-harness-process-that-is-gone).
...
- `continuity`: every iteration spawns a new worker session either way; this chooses its checkout. `accumulate` (default) continues in the previous attempt's worktree and branch so progress and the metric are cumulative. `fresh_attempt` gives each attempt a clean tree, so iterations are independent and the metric restarts from the baseline. Keep the default for reviewer-judged goals: each round should fix the reviewer's points on the same branch, not start over.
...
Killing is an immediate stop-and-remove operation. It cascades lifecycle state downward, stops attached harness sessions, and removes the worker or target subtree from active state in the same command, so killed records do not linger in the AgentTree. A killed worker's managed git worktree is removed in the same pass, reusing the same safe cleanup path as `/prune`: only a clean, unlocked, unshared worktree the killed worker owns is removed (with `git worktree remove`, never `--force`), the delivery branch is retained so committed work stays reachable, and a worktree another live or queued agent still shares is left in place and reported as retained. Dirty, locked, or ambiguous worktrees are preserved with a warning. `/prune` remains the lifecycle cleanup command for eligible completed/errored records, and startup reconciliation applies the same safe worktree cleanup to any killed records left by an interrupted command.
...
- The persisted worktree path must still be registered to the persisted managed branch in the expected repository and must not be the main checkout or overlap a path referenced by another worker. Historical prefixed and hash-suffixed branch names are not allocator-managed and are never adopted or cleaned by workspace lifecycle operations.
...
Compacting reuses ids, so the pass also resolves every id already written about a record: references and narrative text follow a renamed record, and an id whose record is gone is cleared in live slots, marked `(old id)` in history slots, and marked in prose only when the pass hands that spelling to a surviving record or the kernel stored that spelling in a slot, so it cannot be read as the record that inherited the id while ordinary text that merely looks like an id (`Q3 revenue`) is left alone. See `docs/recount.md`.
...
Selects the active harness backend for future heads and workers, backing `/harness <pi|claude|codex>`.
