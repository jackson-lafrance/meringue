...
- Interactive terminal behavior that needs a live TTY/PTY: raw-mode key handling, PTY echo timing, mouse events, and true-color rendering fidelity. Rendering logic is tested through the pane/canvas objects instead. For example `test/integration/tui/chat_target_composer_test.rb` asserts which style the composer's chat-target tint resolves to in every colorscheme, while judging how that color actually looks in your terminal is a manual step (`ruby scripts/chat_target_smoke.rb` prints the composer for every selection state and theme).
