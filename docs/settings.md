...
First-run Setup is a curated `Settings::Draft` mode, not a second persistence implementation. It presents a centered, welcoming Welcome → Theme → Head defaults → Worker defaults → Status bar → Meringue Xtras flow with one dynamic step indicator, contextual Enter/picker controls, and a restrained optional welcome animation. The status-bar composer returns its layout to the same draft, and Meringue Xtras ends the flow with Complete. It reuses the schema, editors, validation, theme and status previews, hit testing, and persistence result handling without inheriting the dense advanced-settings presentation.
