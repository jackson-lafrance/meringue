...
head_extra_args = ["--model", "openai/gpt-5.6-sol", "--thinking", "max", "--tools", "read,bash,grep,find,ls"]
worker_extra_args = ["--model", "openai/gpt-5.6-sol", "--thinking", "max", "--tools", "read,bash,grep,find,ls,edit,write"]
...
Pi heads and workers default to `openai/gpt-5.6-sol` at Pi's maximum thinking level (`--thinking max`). To use a different model or thinking level, set `head_extra_args` / `worker_extra_args` for `[harness.pi]`; a configured array replaces the default array entirely, so include the other flags you still want.
