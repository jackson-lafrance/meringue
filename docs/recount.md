...
Prose is audited more narrowly, because a bare prose token that names no record is legal there (see below) and, once the pass is done, an unmarked reused spelling cannot be told apart from the rewrite's own output. What can still be proven after the fact is a token that names no record now and that the state itself vouched for as an ID before the pass — a record this pass renamed away, or a value the kernel held in a reference slot — because nothing legitimate can write it; its presence means the rewrite skipped a token and the pass fails.
...
- in narrative text, the ID is marked `(old id)` only on evidence that the token is a reference and not the user's own words. Two things count as evidence. This pass hands that exact spelling to a surviving record: `Worker P2-I2-W1 (old id) completed.` when the live World worker is being renumbered to `P2-I2-W1`, because left bare the line would read as the surviving worker's history. Or the kernel itself stored that spelling in a reference slot somewhere in state — the prune log's `removed_issue_ids`, a log's `source_id`, a lineage field — which is how `Pruned issue P2-I3.` keeps its marker even when nothing takes `P2-I3` in this pass, so the next issue created after the recount does not inherit that line. The kernel only writes an ID into a slot when it means a record, so the same token in prose is a reference too. A token with neither kind of evidence is left exactly as written: nothing in state can tell a never-recorded `Q3` from the user's own `Prepare Q3 revenue report`, and a recount that renamed nothing near it has no business editing prose. Marking every such token used to mis-edit ordinary text on every recount, which was the reported bug.
...
After renumbering, the pass validates that every AgentTree ID held in a reference slot names a record that exists right now, and that no prose still spells an ID the pass renamed away or retired from a slot. If anything is left unresolved the whole recount is aborted and nothing is persisted, because persisting it would mean persisting misattributed history.
...
An ID used illustratively rather than as a reference (an issue title that spells out `P3 before P3-I10`) is treated like any other prose token: it is renamed if that record exists, marked `(old id)` if this pass gives its spelling to another record or the kernel stored that spelling in a slot, and otherwise left as written. Nothing in the token itself distinguishes the two, so the pass edits an example only when leaving it alone could misattribute history.
