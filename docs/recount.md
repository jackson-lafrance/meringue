...
- in append-only history (logs, log details, chat messages) and in narrative text, the ID is marked `(old id)`: `Worker P2-I2-W1 (old id) completed.` The line still says what happened, it stays attributed to nothing rather than to the wrong record, and the annotated value no longer matches any record ID, so log attribution, `GetInfo`, the focused-workspace transcript filter, and ID completion all stop resolving it.
...
After renumbering, the pass validates that every AgentTree ID still spelled anywhere in state names a record that exists right now. If anything is left unresolved the whole recount is aborted and nothing is persisted, because persisting it would mean persisting misattributed history.
...
- Log IDs remain append-only, and conversation message IDs remain unchanged. Only the AgentTree IDs *inside* those records are resolved.
...
An ID used illustratively rather than as a reference (an issue title that spells out `P3 before P3-I10`) is treated like any other reference: it is renamed if that record exists and marked `(old id)` if it does not. Nothing in state distinguishes the two, and misattributing history is worse than annotating an example.
