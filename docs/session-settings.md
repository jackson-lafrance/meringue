...
`/model` and `/thinking` save harness-neutral values in Meringue's configured TOML file (normally `~/.meringue/config.toml`), while mirroring the legacy `[harness.pi]` layout for migration compatibility. The one-argument `/model <provider>/<model-id>` and `/thinking <level>` forms keep their historical behavior: they update both roles and clear role-specific overrides. The `head` and `worker` forms update only that role, including sessions spawned later in the currently running Meringue process. The `agent_defaults_mode` experiment controls whether role-specific overrides are active; it defaults to `role-specific` and is listed in `/config` under Experiments. In `shared` mode there is one value for both roles, so a role-named write updates both and the pickers show no Head/Worker tabs.
