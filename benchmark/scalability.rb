...
module ScalabilityBenchmark
  ...
  class ProtocolTerminal
    ...
    def initialize(input: $stdin, output: $stdout, on_sync: nil)
      ...
      # Drive the real differential terminal writer into an in-memory sink. The
      # protocol still sends acknowledgements over stdout, but measured latency
    ...
    def write_frame(frame)
      ...
      @output.puts(JSON.generate(
        ...
        "ansi_color" => frame.include?("\e["), "terminal_bytes" => terminal_bytes,
  ...
  def run_one(count:, samples:, interval:, payload_bytes:)
    Dir.mktmpdir("meringue-scale-") do |dir|
      ...
      typing = []
      scrolling = []
      typing_terminal_bytes = []
      scrolling_terminal_bytes = []
      peak_rss_kb = 0
      summary = nil
      initial_terminal_bytes = nil
      ...
      Open3.popen3({ "NO_COLOR" => nil }, *command) do |stdin, stdout, stderr, wait|
        ...
        initial_terminal_bytes = initial.fetch("terminal_bytes")
        ...
        samples.times do
          response = send_input.call((sequence % 26 + 97).chr, typing)
          typing_terminal_bytes << response.fetch("terminal_bytes")
        end
        ...
        samples.times do |index|
          ...
          scrolling_terminal_bytes << response.fetch("terminal_bytes")
      ...
      file_state = JSON.parse(File.read(path))
      {
        ...
        "terminal_output_bytes" => {
          "full_frame" => initial_terminal_bytes,
          "typing_max" => typing_terminal_bytes.max,
          "scrolling_max" => scrolling_terminal_bytes.max
        },
  ...
  def parent(options)
    ...
    output = {
      "methodology" => "separate process; input write through ANSI rendering and production terminal diff to frame acknowledgement; fake harness; concurrent isolated-state reconciliation",
