    ...
    class App
      ...
      def keybinding_help_text
        <<~TEXT.strip
          ...
          Slash commands: type / for suggestions; #{keys_for("focus_next")} / #{keys_for("focus_previous")} cycle the visible list forward/backward; #{keys_for("suggestion_previous")}/#{keys_for("suggestion_next")} also move it; #{keys_for("submit")} applies the selected suggestion, or sends the prompt as typed when none is selected.
