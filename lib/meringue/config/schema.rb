    ...
    module Schema
      ...
      def add_role_defaults(settings)
        settings.concat([
          ...
          # Shared model and reasoning defaults are the normal setup surface. The role-specific
          # values remain available when the split-defaults experiment is enabled, while these
          # fields preserve one setting for both roles and read older config spellings as fallbacks.
          definition("agent.model", %w[harness model], "Agent defaults", "model_reference", "anthropic/claude-opus-5", fallback_paths: [%w[harness pi model], %w[harness head_model], %w[harness worker_model], %w[harness pi head_model], %w[harness pi worker_model]], editor: "model", apply_mode: "live", label: "Model", description: "Shared model for future heads and workers; exact provider/model references remain allowed when the catalog is unavailable."),
          definition("agent.thinking", %w[harness thinking_level], "Agent defaults", "thinking_level", "max", fallback_paths: [%w[harness pi thinking_level], %w[harness head_thinking_level], %w[harness worker_thinking_level], %w[harness pi head_thinking_level], %w[harness pi worker_thinking_level]], options: THINKING_LEVELS, editor: "selector", apply_mode: "live", label: "Thinking level", description: "Shared reasoning level for future heads and workers."),
          ...
          definition("agent.head_model", %w[harness head_model], "Agent defaults", "model_reference", "anthropic/claude-opus-5", fallback_paths: [%w[harness model], %w[harness pi head_model], %w[harness pi model]], editor: "model", apply_mode: "live", label: "Head model", description: "Model for future heads; shown when split head and worker defaults are enabled."),
          definition("agent.head_thinking", %w[harness head_thinking_level], "Agent defaults", "thinking_level", "max", fallback_paths: [%w[harness thinking_level], %w[harness pi head_thinking_level], %w[harness pi thinking_level]], options: THINKING_LEVELS, editor: "selector", apply_mode: "live", label: "Head reasoning", description: "Reasoning level for future heads; shown when split head and worker defaults are enabled."),
          definition("agent.worker_model", %w[harness worker_model], "Agent defaults", "model_reference", "anthropic/claude-opus-5", fallback_paths: [%w[harness model], %w[harness pi worker_model], %w[harness pi model]], editor: "model", apply_mode: "live", label: "Worker model", description: "Model for future workers; shown when split head and worker defaults are enabled."),
          definition("agent.worker_thinking", %w[harness worker_thinking_level], "Agent defaults", "thinking_level", "max", fallback_paths: [%w[harness thinking_level], %w[harness pi worker_thinking_level], %w[harness pi thinking_level]], options: THINKING_LEVELS, editor: "selector", apply_mode: "live", label: "Worker reasoning", description: "Reasoning level for future workers; shown when split head and worker defaults are enabled."),
