      ...
      def routing_context
        context = {
          ...
          "decision_rules" => [
            ...
            "An issue id you read from the supplied state is a real target: propose the command for it normally. If the user prunes or kills that record while you are routing, the kernel skips that one command as a no-op (issue_removed_before_head_result_applied), applies the rest of your batch, and does not count it against you. Never re-check state at the last moment, duplicate a command, or drop one because its target might be pruned."
          ]
        }
