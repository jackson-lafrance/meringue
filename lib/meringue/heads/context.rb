    ...
    class Context
      ...
      def system_prompt
        <<~PROMPT
          ...
          An issue is the durable goal and a worker is one session step, so one goal that needs research and then implementation is one issue with two workers on it (issue_from_command on both, plus after_from_command and follow_up_of_command on the implementer), not two issues. Create a second issue only for a genuinely independent goal. Never write a worker prompt that polls Meringue state or sleeps waiting for another worker: the kernel owns that wait. When a step must wait for something outside Meringue instead - a review landing on a PR, a deploy, an external job - queue it with after_command and let the kernel poll that command. When completion-head routing itself must wait and the condition is known up front, put after_command inside the completion_head object so the kernel holds the continuation without spawning a checker worker.
      ...
      def routing_context
        context = {
          ...
          "decision_rules" => [
            ...
            "PromptAgent targets a worker. Retrying a head is an explicit user action (/retry) that starts a fresh head, so never propose PromptAgent or RetryHead on a head id; those commands are rejected for heads.",
            ...
            "Use after_command when the next step must wait for something outside Meringue rather than for an agent: a review landing on a PR, a deploy going green, an external job finishing. The kernel polls that command on a timer (exit 0 by default, or after_command_expect: \"output_matches\" with after_command_pattern), starts the worker when it passes, and hands over the command's last output. Give it a short after_command_label so the AgentTree and logs read honestly. after_command composes with after_agent_id as AND, and the command is only polled once the predecessor settles, which is the right shape for \"open the PR, wait for the pair review, then respond to it\". If it never passes within after_command_max_wait_seconds (4h default) the worker is cancelled with a warning unless if_gate_expires is \"run\".",
            "Use completion_head.after_command when the dynamic routing decision itself must wait for an external condition that is already known when the worker is spawned. The worker may complete and open its PR, but the kernel keeps the completion continuation queued on that completed worker until the condition passes; no head or checker worker is spawned early. It uses the same interval, timeout, max-wait, output matching, cwd, and expiry policy as an ordinary after_command gate. If a completion head only discovers the condition from the final report, have it queue the real follow-up SpawnWorker with after_command instead of launching a short-lived worker just to check the state and chain another completion head.",
            "Never write a worker prompt that polls or sleeps waiting for an external condition either. If the step needs to wait for a script, a review, or a job, express it as after_command on the queued worker or completion continuation; the kernel owns the polling, the budget, and the handover.",
