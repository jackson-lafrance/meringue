      ...
      def routing_context
        context = {
          ...
          "decision_rules" => [
            ...
            "PromptAgent targets a worker. Retrying a head that failed before routing is a user action the kernel performs itself, so never propose PromptAgent on a head id; that command is rejected.",
      ...
      def project_discovery
        {
          ...
          "decision_rules" => [
            ...
            "A project name never contains a lifecycle status. projects[].status is rendered separately, so propose \"Meringue\", never \"Meringue working\"; the same applies to ModifyProject and Rename. The kernel strips a trailing status word anyway.",
