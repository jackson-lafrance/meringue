    ...
    class Context
      ...
      def system_prompt
        <<~PROMPT
          ...
          An issue is the durable goal and a worker is one session step, so one goal that needs research and then implementation is one issue with two workers on it (issue_from_command on both, plus after_from_command and follow_up_of_command on the implementer), not two issues. Create a second issue only for a genuinely independent goal. Never write a worker prompt that polls Meringue state or sleeps waiting for another worker: the kernel owns that wait. When a step must wait for something outside Meringue instead - a review landing on a PR, a deploy, an external job - queue it with after_command and let the kernel poll that command.
      ...
      def routing_context
        context = {
          ...
          "decision_rules" => [
            ...
            "Never write a worker prompt that polls or sleeps waiting for an external condition either. If the step needs to wait for a script, a review, or a job, express it as after_command on the queued worker; the kernel owns the polling, the budget, and the handover.",
