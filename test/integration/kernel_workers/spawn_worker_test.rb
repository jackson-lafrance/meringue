...
class KernelWorkersSpawnTest < Minitest::Test
  ...
  def test_spawn_allocates_a_workspace_and_hands_it_to_the_harness_as_cwd
    ...
    assert_includes spawn_call.fetch("system_prompt"), "Before every push, fetch the target base branch"
    assert_includes spawn_call.fetch("system_prompt"), "even when the pull request reports no conflicts"
    assert_includes spawn_call.fetch("system_prompt"), "If the base is unchanged, do not rebase"
    assert_includes spawn_call.fetch("system_prompt"), "Never force-push a worker branch"
    assert_includes spawn_call.fetch("system_prompt"), "preserve the workspace"
