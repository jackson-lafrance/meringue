...
class HarnessRegistryTest < HarnessIntegrationTest
  ...
  def test_normalize_provider_bang_rejects_unknown_harnesses
    ...
    assert_match(/pi, claude, codex/, error.message)
  end
  ...
  def test_provider_metadata_is_public_facing
    assert_equal %w[pi claude codex], Registry.supported_provider_names
    ...
    assert_equal %w[pi claude codex], choices.map { |choice| choice.fetch("provider") }
    assert_equal %w[pi claude codex], choices.map { |choice| choice.fetch("internal_provider") }
    assert_equal ["Pi", "Claude Code", "Codex CLI"], choices.map { |choice| choice.fetch("label") }
  ...
  def test_client_for_builds_interactive_clients
    subject = registry(
      "harness" => {
